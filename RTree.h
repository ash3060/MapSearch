...
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(Rect* a_rectA, Rect* a_rectB)
{
  ...
  for(int index = 0; index < NUMDIMS; ++index)
  {
    newRect.m_min[index] = (a_rectA->m_min[index] < a_rectB->m_min[index]) ? a_rectA->m_min[index] : a_rectB->m_min[index]; //min
    newRect.m_max[index] = (a_rectA->m_max[index] > a_rectB->m_max[index]) ? a_rectA->m_max[index] : a_rectB->m_max[index];  //max
  }
    ...
    for(int index=0; index < a_node->m_count; ++index)
    {
      if(Overlap(a_rect, &a_node->m_branch[index].m_rect))
      {
        ...
        if(&a_resultCallback)
        {
          ...
          //fprintf(stderr, "\nleaf POI %d (%lf, %lf)\n", id, a_node->m_branch[index].m_rect.m_min[0], a_node->m_branch[index].m_rect.m_min[1]);
        }
      }
    }
  }
